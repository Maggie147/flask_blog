# -*- coding: utf-8 -*-
from flask import request, render_template, flash, abort, url_for, redirect, session, g, current_app, make_response
from flask_login import login_required, current_user
from flask import escape, jsonify, make_response
from datetime import datetime
from functools import wraps  # @wraps 是一个装饰器
        ...
        def wrapper(*args, **kw):
            for arg in required_args:
                if arg not in request.json:
                    return jsonify({'code': 400, 'msg': u'参数不对'})
            return func(*args, **kw)
        return wrapper
    return decorator()
...
def post(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('You comments has been publish.')
